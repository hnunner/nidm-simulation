...
exportRegressionModels <- function(ssData = loadSimulationSummaryData()) {
  ...
  omega                              <- meanCenter(ssData$cr.omega)
  delta                              <- meanCenter(ssData$cr.delta)
  c                                  <- meanCenter(ssData$cr.c)
  N                                  <- meanCenter(ssData$cr.N)
  iota                               <- meanCenter(ssData$cr.iota)
  phi                                <- meanCenter(ssData$cr.phi)
  # dependent variables
  ...
  ### LINEAR REGRESSIONS ###
  # stability
  regStable <- lm(stable ~ omega + delta + c + N + iota + phi, data = ssData)
  # average degree
  regDegree <- lm(av.degree ~ omega + delta + c + N + iota + phi, data = ssData)
  # average clustering
  regClustering <- lm(av.clustering ~ omega + delta + c + N + iota + phi, data = ssData)
  # average path length
  regPathLength <- lm(av.pathlength ~ omega + delta + c + N + iota + phi, data = ssData)
...
exportSummary <- function(ssData = loadSimulationSummaryData()) {
  ...
  filepathByN <- paste(EXPORT_PATH_NUMERIC,
                       ...
                       EXPORT_FILE_EXTENSION_SUMMARY,
                       sep = "")
  write.csv(do.call("rbind",(describeBy(ssData, group = ssData$cr.N))), filepathByN)
}
...
getPlots <- function(ssData = loadSimulationSummaryData(), includeStability = TRUE, includeN = TRUE) {
  ...
  plots <- getPlotRow(ssData,
                      ssData$cr.omega,
                      expression(paste("Social benefits (", omega,")",sep="")),
                      1,
                      includeStability)

  plots <- c(plots, getPlotRow(ssData,
                               ssData$cr.delta,
                               expression(paste("Benefit discount (", delta,")",sep="")),
                               1,
                               includeStability))

  plots <- c(plots, getPlotRow(ssData,
                               ssData$cr.c,
                               expression(paste("Social costs (", c,")",sep="")),
                               1,
                               includeStability))
...
exportPlots <- function(ssData = loadSimulationSummaryData(),
  ...
  ggsave(paste(EXPORT_PATH_PLOTS,
               ...
               sep = ""),
         do.call("grid.arrange",
                 c(getPlots(ssData = subset(ssData, ssData$cr.N == 20),
                            includeN = FALSE),
                   ncol=4)),
         width = plotWidth,
         height = plotHeight,
  ...
  ggsave(paste(EXPORT_PATH_PLOTS,
               ...
               sep = ""),
         do.call("grid.arrange",
                 c(getPlots(ssData = subset(ssData, ssData$cr.N == 20 & ssData$net.stable == 1),
                            includeN = FALSE,
                            includeStability = FALSE),
                   ncol=3)),
         width = plotWidth,
