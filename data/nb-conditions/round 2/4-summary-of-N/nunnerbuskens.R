...
plot_exclusions = function(data) {
  ggplot(data, aes(x = factor(data$alpha),
    ...
    labs(fill = "alpha",
         x = "alpha",
         y = paste("exclusions (N = ", data$N[1], ")", sep = "")) +
...
plot_densities_degrees = function(data) {
  ggplot(data, aes(x=data$degrees, fill=factor(data$alpha))) +
    ...
    labs(fill = "alpha",
         x = "av. degree",
         y = paste("density (N = ", data$N[1], ")", sep = "")) +
    scale_x_continuous(limits=c(6,  8), breaks=seq(6, 8,0.25)) +
...
plot_densities_path_length = function(data) {
  ggplot(data, aes(x=data$path.lengths, fill=factor(data$alpha))) +
    ...
    labs(fill = "alpha",
         x = "av. path length",
         y = paste("density (N = ", data$N[1], ")", sep = "")) +
    scale_x_continuous(limits=c(1,  4.5), breaks=seq(1, 4.5,0.5)) +
...
exportAlphaAnalyses <- function() {
  ...
  for (N in unique(data$nb.N)) {

    data.by.N        <- subset(data, data$nb.N == N)

    for (alpha in unique(data.by.N$nb.alpha)) {

      data.by.N.alpha   <- subset(data.by.N, data.by.N$nb.alpha == alpha)
      n                 <- nrow(data.by.N.alpha)

      data.by.N.alpha   <- subset(data.by.N.alpha, data.by.N.alpha$net.pathlength.av > 1 & data.by.N.alpha$net.stable == 1)
      exclusions        <- (n - nrow(data.by.N.alpha)) / n

      Ns                <- rep(N, nrow(data.by.N.alpha))
      alphas            <- rep(alpha, nrow(data.by.N.alpha))

      degrees           <- data.by.N.alpha$net.degree.av
      degrees.mean      <- mean(degrees)
      degrees.sd        <- sd(degrees)

      clusters          <- data.by.N.alpha$net.clustering.av
      clusters.mean     <- mean(clusters)
      clusters.sd       <- sd(clusters)

      path.lengths      <- data.by.N.alpha$net.pathlength.av
      path.lengths.mean <- mean(path.lengths)
      path.lengths.sd   <- sd(path.lengths)

      densities         <- data.by.N.alpha$net.density
      densities.mean    <- mean(densities)
      densities.sd      <- sd(densities)

      data.summary      <- rbind(data.summary, data.frame(
        N = unique(Ns), alpha = unique(alphas),
        n, exclusions,
  ...
  p.exclusions <- do.call("ggarrange",
                          c(lapply(X = split(data.summary, data.summary$N),
                                   FUN = plot_exclusions),
  ...
  data.plots.split <- split(data.plots, data.plots$N)
  ### AVERAGE DEGREES
