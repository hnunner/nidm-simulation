...
exportRegressionModelsComplete <- function(ssData = loadSimulationSummaryData()) {
  ...
  index.degree                       <- meanCenter(ssData$index.degree / (ssData$cidm.N-1))
  ...
  ### level 1: simulation runs                    ###
  # null-model
  reg00    <- lmer(ssData$sim.epidemic.duration ~
                      1 +
                      (1 | sim.upc),
                    data = ssData,
                    REML = FALSE)
  # main effects: varied CIDM parameters
  reg1Main <- lmer(ssData$sim.epidemic.duration ~
                      #  model parameters
                      beta +
                      mu +
                      sigma +
                    ...
                    REML = FALSE)
  # network properties
  reg2Main <- lmer(ssData$sim.epidemic.duration ~
                      #  model parameters
                      beta +
                      mu +
                      sigma +
                    ...
                    REML = FALSE)
  # interaction effects
  reg2Int  <- lmer(ssData$sim.epidemic.duration ~
                      #  model parameters
                      beta +
                      mu +
                      sigma +
