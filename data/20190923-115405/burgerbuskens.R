...
exportRegressionModels <- function(ssData = loadSimulationSummaryData()) {
  ...
  # average degree
  regDegree <- lm(av.degree ~ b1 + b2 + c1 + c2 + c3 + N + iota + phi, data = ssData)
  # average clustering
  regClustering <- lm(av.clustering ~ b1 + b2 + c1 + c2 + c3 + N + iota + phi, data = ssData)
  # average path length
  regPathLength <- lm(av.pathlength ~ b1 + b2 + c1 + c2 + c3 + N + iota + phi, data = ssData)
...
getPlots <- function(ssData = loadSimulationSummaryData(), includeStability = TRUE) {
  ...
  plots <- c(plots, getPlotRow(ssData,
                               ssData$bb.b2,
                               expression(paste("Triadic closure benefits (", b[2],")",sep="")),
                               ssData$bb.c3*2,
                               includeStability))

  plots <- c(plots, getPlotRow(ssData,
                               ssData$bb.c3,
                               expression(paste("Triadic closure costs (", c[3],")",sep="")),
                               ssData$bb.b2*2,
                               includeStability))
